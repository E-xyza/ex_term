(() => {
    ...
    const fetchCols = (row, [row1, col1], [row2, col2]) => {
        ...
        while (true) {
            ...
            if ((!element) || (element.className.includes("exterm-cell-sentinel"))) {
                if (last_empty) {
                    return line + "\n";
                } else {
                    return line;
            ...
            last_empty = element.className.includes("exterm-cell-blank");
            ...
            if (row == row2 && index == col2) return line;
            index += 1;
        }
    }
    ...
    const modifyClipboard = (event) => {
        const xterm_console = document.getElementById("exterm-console");
